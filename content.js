...
theory:`
<p>🎯 <b>Зачем это тебе:</b> заказчик почти никогда не хочет «блоки столбиком» — он хочет карточки товаров в три колонки, меню в одну строку, галерею ровной сеткой. Именно раскладка отличает страницу-черновик от макета, за который платят. Сегодня освоишь два инструмента, на которых держится вся современная вёрстка.</p>
<p>Сначала — почему всё падает вниз. Каждый <code>div</code>, заголовок и абзац — <b>блочный элемент</b>: он занимает всю ширину страницы и сталкивает соседа вниз. Сколько бы карточек ты ни написал, сами по себе они выстроятся в столбик. Чтобы поставить их рядом, включают <b>раскладку</b> (layout — правило, по которому браузер расставляет элементы), причём включают её не на самих карточках, а на их родителе-контейнере.</p>
<p><b>Flexbox</b> (flex — гибкий) — раскладка в одну линию:</p>
<pre class="demo">.menu {
  display: flex;   /* дети встают в ряд */
  gap: 12px;       /* промежуток между ними */
}</pre>
<p>Разбор: <code>display: flex</code> пишут контейнеру — и все его прямые дети выстраиваются в ряд слева направо. <code>gap</code> (промежуток) задаёт расстояние между ними одним свойством: не нужно вешать margin на каждый элемент и воевать с лишним отступом у последнего.</p>
<p>Направление и выравнивание:</p>
<pre class="demo">.hero {
  display: flex;
...
<p>Разбор: <code>flex-direction</code> задаёт направление ряда: <code>row</code> — по горизонтали (так по умолчанию), <code>column</code> — по вертикали. <code>justify-content</code> распределяет элементы вдоль главной оси — той, куда направлен флекс: значение <code>center</code> собирает всё в центре, а <code>space-between</code> прижимает крайние элементы к краям, укладывая свободное место между ними — так делают шапки сайтов: логотип слева, кнопка справа. <code>align-items: center</code> выравнивает по поперечной оси: иконка и текст рядом с ней встают на одну среднюю линию. Этих значений хватает на девять заказов из десяти.</p>
<p><b>Grid</b> (сетка) — раскладка сразу и по колонкам, и по строкам:</p>
<pre class="demo">.cards {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 16px;
}</pre>
<p>Разбор: <code>display: grid</code> включает сетку на контейнере. <code>grid-template-columns</code> описывает колонки: запись <code>repeat(3, 1fr)</code> значит «повтори колонку шириной 1fr три раза». Единица <code>fr</code> (fraction — доля) — доля свободной ширины: три колонки по 1fr всегда равны и сами растягиваются под экран, никаких пикселей подгонять не надо. Дети контейнера раскладываются по клеткам автоматически: четвёртая карточка сама перейдёт на новую строку. <code>gap</code> работает как во флексе, только сразу в обе стороны — и между колонками, и между строками.</p>
<p>Как выбрать инструмент? Запоминалка: <b>flex — лента, grid — сетка</b>. Одна линия элементов — меню, шапка, ряд кнопок — это flex. Плитки в несколько строк и колонок — карточки товаров, галерея работ, каталог — это grid. Ошибиться не страшно: почти любую раскладку можно собрать обоими способами, просто правильный инструмент даёт код короче.</p>
<p>Осталась <b>адаптивность</b> — умение страницы перестраиваться под ширину экрана: заказчик первым делом откроет твою вёрстку с телефона. Для этого есть <b>медиазапрос</b> — условие «если экран такой-то, применяй другие стили»:</p>
<pre class="demo">@media (max-width: 600px) {
  .cards { grid-template-columns: 1fr; }
}</pre>
<p>Разбор: <code>@media (max-width: 600px)</code> читается «если экран не шире 600 пикселей». Внутри фигурных скобок — обычные CSS-правила: та же сетка .cards получает одну колонку вместо трёх, и карточки на телефоне выстраиваются в столбик во всю ширину. Одна строка кода — и в переписке с заказчиком можно честно писать «адаптив есть».</p>
<p>⚠️ <b>Частые ошибки:</b></p>
...
quiz:[
 ...
 {t:"bug",q:"В какой строке ошибка?",code:[".cards {","  display: grid;","  grid-template-column: repeat(3, 1fr);","  gap: 16px;","}"],a:2,e:"Свойство называется grid-template-columns — с s на конце. Браузер не покажет ошибку, просто молча пропустит строку, и колонки не появятся."},
...
practice:{type:"html",
task:`<p><b>Что делаем:</b> «заказ» от магазина — три карточки товаров, выстроенные в ровную сетку с промежутками.</p><p><b>Шаги:</b></p><ol><li>Внутри &lt;body&gt; создай контейнер &lt;div class="cards"&gt; и положи в него три карточки &lt;div class="card"&gt; — в каждой название товара и цена.</li><li>В &lt;style&gt; напиши правило для .cards: display: grid и grid-template-columns: repeat(3, 1fr) — или display: flex, если хочешь ряд.</li><li>Там же задай промежуток между карточками: gap: 16px.</li><li>Оформи .card, чтобы карточки было видно: например background: #f0f0f0; padding: 16px;</li><li>Нажми «Запустить» — три карточки должны стоять в ряд с равными промежутками.</li></ol>`,
starter:"<!DOCTYPE html>\n<html>\n<head>\n<style>\n  /* TODO: правило для .cards — включи сетку или флекс и задай промежуток */\n\n  /* TODO: правило для .card — фон и внутренний отступ */\n</style>\n</head>\n<body>\n  <h1>Наши товары</h1>\n\n  <!-- TODO: контейнер cards, внутри — три карточки card с товаром и ценой -->\n\n</body>\n</html>",
checks:[
 {t:"CSS написан внутри тега &lt;style&gt;",fn:c=>/<style[\s>][\s\S]*<\/style>/i.test(c)},
 {t:"Контейнеру задан display: flex или display: grid",fn:c=>/display\s*:\s*(flex|grid)/i.test(c)},
 {t:"Задан промежуток gap",fn:c=>/gap\s*:/i.test(c)},
 {t:"Минимум три элемента с атрибутом class",fn:c=>(c.match(/class\s*=/gi)||[]).length>=3},
 {t:"В CSS есть селектор класса (через точку)",fn:c=>{const s=(c.match(/<style[\s>]([\s\S]*?)<\/style>/i)||[])[1]||"";return /\.[a-z][\w-]*\s*\{/i.test(s);}}],
hint:"В <style>: .cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; } — и три <div class=\"card\">…</div> внутри <div class=\"cards\">."}},
...
<p><b>Форма</b> — область страницы, куда посетитель вводит данные:</p>
<pre class="demo">&lt;form&gt;
  &lt;label for="name"&gt;Ваше имя&lt;/label&gt;
  &lt;input type="text" id="name"&gt;
  &lt;button type="submit"&gt;Отправить&lt;/button&gt;
&lt;/form&gt;</pre>
<p>Разбор построчно: <code>&lt;form&gt;</code> — контейнер, который объединяет поля в одну заявку. <code>&lt;label&gt;</code> — подпись к полю; её атрибут <code>for="name"</code> ссылается на поле с <code>id="name"</code> (id — уникальное имя элемента, на странице не повторяется) — так браузер понимает, что подпись и поле — пара. <code>&lt;input&gt;</code> — само поле ввода; как img, это одиночный тег без закрывающей пары. <code>&lt;button type="submit"&gt;</code> — кнопка, которая отправляет всё, что введено в форму.</p>
<p>Каким будет поле, решает атрибут <code>type</code>:</p>
...
<p>Разбор: <code>text</code> — универсальное текстовое поле. <code>email</code> — браузер сам проверит, что внутри есть @, и не даст отправить ерунду, а на телефоне покажет клавиатуру с этим символом. <code>number</code> — пускает только цифры и добавляет стрелочки. <code>checkbox</code> — квадратик-галочка, например «согласен на звонок». Правильный type — бесплатная проверка данных без единой строки JavaScript: у заказчика меньше мусорных заявок.</p>
<p>Для длинного текста — пожеланий к брони, текста отзыва — есть <code>&lt;textarea&gt;</code>:</p>
<pre class="demo">&lt;label for="wish"&gt;Пожелания&lt;/label&gt;
&lt;textarea id="wish"&gt;&lt;/textarea&gt;</pre>
<p>Разбор: в отличие от input это парный тег — между открывающим и закрывающим можно положить текст-заготовку. Поле многострочное, пользователь может растянуть его мышкой под длинный текст.</p>
<p>Почему label — не украшение, а обязательный элемент. Связка for + id делает подпись кликабельной: тап по слову «Согласен» ставит галочку в чекбокс, а попасть пальцем по слову куда проще, чем по крошечному квадратику. Второе: экранный диктор (программа, которая читает страницу незрячим) произнесёт «Ваше имя, поле ввода» — без label он скажет просто «поле ввода», и человек не поймёт, что вводить. Форма без label — брак в работе.</p>
<p>Теперь <b>семантический HTML</b> (семантика — смысл: тег сообщает, чем блок является). Страницу собирают не из безликих div, а из смысловых зон:</p>
...
<p>Разбор построчно: <code>&lt;header&gt;</code> — шапка страницы: логотип, название. <code>&lt;nav&gt;</code> (navigation — навигация) — блок со ссылками-меню. <code>&lt;main&gt;</code> — главное содержимое; такой тег на странице ровно один. <code>&lt;section&gt;</code> — смысловой раздел внутри: «о нас», «форма заявки». <code>&lt;footer&gt;</code> — подвал: контакты, копирайт. Ведут они себя как обычные div — «коробки» без собственного вида, красить их можно теми же CSS-правилами.</p>
<p>Что это даёт, если на вид разницы нет? Три вещи. Поисковики (отсюда SEO — оптимизация под поисковую выдачу) понимают, где на странице главное, и охотнее поднимают сайт в результатах. Незрячим пользователям диктор даёт прыгнуть сразу к main, не слушая шапку заново на каждой странице. И читаемость: в чужом коде &lt;footer&gt; понятен мгновенно, а &lt;div class="block27"&gt; — тайна. «Див-суп» — страница из сплошных div — первое, за что ругают вёрстку новичка.</p>
<p>⚠️ <b>Частые ошибки:</b></p>
...
quiz:[
 ...
 {t:"bug",q:"В какой строке ошибка?",code:["<form>","  <label>Почта</label>","  <input type=\"email\" id=\"mail\">","  <button type=\"submit\">Отправить</button>","</form>"],a:1,e:"У label нет атрибута for=\"mail\" — подпись не связана с полем: клик по слову «Почта» ничего не активирует."},
...
practice:{type:"html",
task:`<p><b>Что делаем:</b> «заказ» от кафе — семантическая страница с формой заявки на бронь столика.</p><p><b>Шаги:</b></p><ol><li>Добавь &lt;header&gt; и внутри него заголовок &lt;h1&gt; с названием кафе.</li><li>Создай &lt;main&gt;, а в нём форму &lt;form&gt;.</li><li>В форме сделай две связки подпись+поле: &lt;label for="…"&gt; + &lt;input type="text"&gt; для имени и &lt;label&gt; + &lt;input type="email"&gt; для почты — не забудь id у полей.</li><li>Добавь &lt;textarea&gt; для пожеланий или галочку &lt;input type="checkbox"&gt; «согласен на звонок», затем кнопку &lt;button type="submit"&gt;.</li><li>Заверши страницу тегом &lt;footer&gt; с телефоном или адресом. Нажми «Запустить» и кликни по подписи — её поле должно активироваться.</li></ol>`,
starter:"<!DOCTYPE html>\n<html>\n<head>\n  <title>Кафе — заявка</title>\n</head>\n<body>\n\n  <!-- TODO: шапка с названием кафе (h1 внутри) -->\n\n  <!-- TODO: основная зона, внутри — форма:\n       две подписи со связкой for+id и два поля (text и email),\n       поле для пожеланий или галочка, кнопка отправки -->\n\n  <!-- TODO: подвал с телефоном или адресом -->\n\n</body>\n</html>",
checks:[
 {t:"Есть форма &lt;form&gt; с закрывающим &lt;/form&gt;",fn:c=>/<form[\s>][\s\S]*<\/form>/i.test(c)},
 {t:"Минимум два поля &lt;input&gt; с атрибутом type",fn:c=>(c.match(/<input[^>]*\btype\s*=/gi)||[]).length>=2},
 {t:"Есть подпись &lt;label&gt;",fn:c=>/<label[\s>]/i.test(c)},
 {t:"Есть &lt;textarea&gt; или чекбокс",fn:c=>/<textarea[\s>]/i.test(c)||/type\s*=\s*["']?checkbox/i.test(c)},
 {t:"Есть кнопка отправки — &lt;button&gt; или type submit",fn:c=>/<button[\s>]/i.test(c)||/type\s*=\s*["']?submit/i.test(c)},
 {t:"Есть семантические теги (header, main, footer…)",fn:c=>/<(header|main|footer|section|nav)[\s>]/i.test(c)}],
hint:"Каркас: <header><h1>Кафе</h1></header>, затем <main><form> <label for=\"name\">Имя</label><input type=\"text\" id=\"name\"> … <button type=\"submit\">Отправить</button> </form></main> и <footer>Контакты</footer>."}},
